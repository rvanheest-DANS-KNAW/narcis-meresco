...
class AddMetadataNamespace(UiaConverter):
    ...
    def _convert(self, lxmlNode):
        ...
        md_format = MetadataFormat(record_lxml, self._uploadid).getFormat() #TODO: pass it somehow from DNA, so we need to look this up only once per record...
        ...
        if len(metapart) > 0: # metapart gevonden...
            ...
            self._insertMetadataNamespace(meta_lxml, md_format)# Check en insert harvestDate...
    ...
    def _insertMetadataNamespace(self, lxmlNode, metadataFormat):
        _bln_has_metadataNamespace = False
